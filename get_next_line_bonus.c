...
/*   Updated: 2022/07/07 11:48:12 by kfortin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	while (read(fd, buf, BUFFER_SIZE) > 0 || ft_strchr(stash[fd]) != -1)
	{
		ft_norminette(&buf, &stash[fd]);
		j = ft_strchr(stash[fd]);
		if (j > -1)
			return (ft_stock_line(&stash[fd], &res, j));
		j = 0;
		buf = ft_calloc(sizeof(char), BUFFER_SIZE + 1);
	}
	free(buf);
	if ((ft_last_line(&stash[fd], &res, j)) < 0)
		return (NULL);
	return (res);
}
